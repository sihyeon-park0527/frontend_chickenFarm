...
### 1️⃣ 실시간 환경 모니터링
![실시간 모니터링](./images/실시간%20자세한%20데이터.gif)
...
- **센서별 상세 히스토리**: 클릭 시 최근 5분 데이터 차트 모달 표시

### 2️⃣ 통계 및 데이터 분석

#### 일간 통계
![일간 통계](./images/통계%20일간.png)

#### 주간 통계
![주간 통계](./images/통계%20주간.png)

#### 월간 통계
![월간 통계](./images/통계%20월간.png)

- **다기간 분석**: 일간/주간/월간 탭 전환으로 다양한 시간 범위 데이터 조회
- **인터랙티브 차트**: Chart.js 기반 라인 차트로 센서 데이터 시각화
- **센서별 상세 분석**: 온도, 습도, 조도, 가스(NH3, CO2, NO2) 개별 차트
- **날짜 선택 기능**: 특정 날짜 및 기간 데이터 조회
- **데이터 비교**: 기간별 트렌드 분석 및 패턴 파악
- **평균/최고/최저값 표시**: 각 센서별 통계 요약 정보 제공

### 3️⃣ 닭 개체 관리
![닭 개체 관리](./images/개체관리.gif)

- **개체 등록 및 조회**: 닭 정보 CRUD
- **상세 정보 관리**: 품종, 입고일, 무게, 건강 상태 등 기록
- **검색 및 필터링**: 개체 번호, 품종별 검색 기능
- **일괄 관리**: 여러 개체 동시 수정 및 삭제
- **예방접종 연계**: 개체별 접종 이력 추적

### 4️⃣ CCTV 통합 모니터링
![CCTV 모니터링](./images/CCTV.gif)

- **실시간 스트리밍**: 양계장 내부 영상 실시간 모니터링
- **영상 기록 재생**: 과거 영상 날짜별 조회 및 재생
- **알림 관리**: 이상 상황 감지 알림 기록 조회
- **다중 뷰 지원**: 여러 카메라 동시 모니터링
- **스냅샷 기능**: 특정 시점 캡처 및 저장

### 5️⃣ 환경 설정 및 임계값 관리
![환경 설정](./images/환경설정.gif)

- **센서 임계값 설정**: 온도, 습도, 가스 센서별 경고/위험 기준값 설정
- **실시간 반영**: 설정 변경 즉시 모니터링 화면에 적용
- **알림 조건 설정**: 임계값 초과 시 알림 발송 조건 관리
- **사육 일지 작성**: 일일 사육 기록 및 메모 관리
- **알림 히스토리**: 과거 센서 알림 내역 조회
...
**주요 요소:**
- 7개 센서 게이지 카드 (온도, 습도, 조도, NH3, CO2, NO2, CO)
- 실시간 값 및 단위 표시
- 환경 점수 (0-100점 종합 평가)
- 날씨 정보 통합 (OpenWeather API)
- 센서 카드 클릭 시 히스토리 차트 모달 (최근 5분 데이터)

**기술 구현:**
- `setInterval` 1초마다 API 폴링 (`/api/realtime`)
- Chart.js 게이지 차트로 시각화
- 임계값 기반 색상 변경 (정상/경고/위험)
- 모달 차트: Chart.js Line Chart
...
**주요 요소:**
- 일간/주간/월간 탭 전환 UI
- 센서별 라인 차트 (온도, 습도, 조도, NH3, CO2, NO2)
- 날짜 선택기 (DatePicker)
- 새로고침 버튼
- 센서별 최신값 요약 카드
- 평균/최고/최저값 표시
...
- Chart.js 멀티 라인 차트
- Day.js 날짜 포맷팅 및 계산
- Material-UI Tabs 컴포넌트
- API: `/api/daily`, `/api/weekly`, `/api/monthly`

---

### 3. 닭 관리 (`/home/chickenmanagement`)
![닭 개체 관리](./images/개체관리.gif)

**주요 요소:**
- 닭 개체 테이블 (개체번호, 품종, 입고일, 무게, 건강상태)
- 개체 추가/수정/삭제 CRUD 기능
- 검색 및 필터링 기능
- 상세 정보 모달

**기술 구현:**
- Material-UI Table 컴포넌트
- RESTful API 연동 (`/api/chickens`)
- React Hook Form 폼 관리

---

### 4. 예방접종 관리
![예방접종 관리](./images/예방접종.gif)
...
**기술 구현:**
- MUI X-Date-Pickers 캘린더
- Day.js 날짜 처리
- API: `/api/inoculations`
...
- 온도: 최저/최고 온도 설정
- 습도: 최저/최고 습도 설정
- 가스: NH3, CO2, NO2, CO 최대값 설정
- 설정 변경 즉시 실시간 모니터링에 반영
